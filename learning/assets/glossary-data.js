...
window.GLOSSARY = {
  ...
  "gc": {
    ...
    en: "cyclic garbage collector",
    def: "Python ปล่อยหน่วยความจำด้วยการนับ reference เป็นหลัก แต่ถ้า object สองตัวชี้หากันเป็นวง ตัวนับจะไม่มีวันถึงศูนย์ จึงต้องมีตัวเก็บกวาดอีกชั้นมาไล่หาวงพวกนี้ — <code>Value</code> กับ <code>_backward</code> ของมันเป็นวงแบบนั้นพอดี เพราะ closure จับ node ที่มันแปะอยู่ไว้ด้วย ผลคือ node ทุกตัวตายช้ากว่าที่ควร"
  }
  ,
  "eps": {
    term: "Machine epsilon",
    en: "machine epsilon",
    def: "ช่องว่างสัมพัทธ์ที่เล็กที่สุดที่ float ชนิดนั้นแยกออก — float32 ราว <code>1.19e-07</code> ส่วน float64 ราว <code>2.22e-16</code> · ตีความว่า “ความละเอียดคิดเป็นเปอร์เซ็นต์” ไม่ใช่จำนวนตำแหน่งทศนิยม จึงเป็นเหตุผลที่ tolerance ในเทสต์ต้องเป็นแบบสัมพัทธ์"
  },

  /* ---- number representation ---- */
  "subnormal": {
    term: "Subnormal",
    en: "subnormal / denormal number",
    def: "เลขที่เล็กเกินกว่าจะเขียนในรูปมาตรฐานได้ จึงยอมทิ้งบิตนำหน้า (กลายเป็น <code>0.</code> แทน <code>1.</code>) และตรึงตัวคูณไว้ที่ <code>2⁻¹²⁶</code> · ทำให้ระยะห่างระหว่างค่าเท่ากันตลอดจนถึงศูนย์ ซึ่งรับประกันว่า <code>a - b == 0</code> ก็ต่อเมื่อ <code>a == b</code> จริง ๆ เรียกว่า gradual underflow"
  },
  "hiddenbit": {
    term: "Hidden leading bit",
    en: "implicit leading bit",
    def: "หลักแรกของ mantissa ที่ไม่ได้ถูกเก็บลงบิตจริง เพราะในฐานสอง “ไม่เป็นศูนย์” มีทางเดียวคือ <code>1</code> · <strong>float32 จึงมีนัยสำคัญ 24 บิต ทั้งที่เก็บจริงแค่ 23</strong> · หายไปเมื่อเป็น subnormal"
  },
  "endian": {
    term: "Endianness",
    en: "byte order / endianness",
    def: "ลำดับที่ไบต์ของเลขหลายไบต์ถูกวางในหน่วยความจำ · <strong>little-endian = ช่องแรกเก็บหลักเล็กสุด</strong> (เหมือนเขียนวันที่ 22/08/2026) · <strong>ไม่ได้แปลว่าอ่านกลับหลัง</strong> และบิตข้างในไบต์ไม่เคยสลับ · เครื่องทุกวันนี้เป็น little แต่เน็ตเวิร์กเป็น big"
  },
  "bf16": {
    term: "bfloat16",
    en: "brain floating point",
    def: "float 16 บิตที่แบ่งบิตให้ exponent เท่า float32 (ช่วง <code>1.18e-38</code> ถึง <code>3.4e38</code>) แลกกับความละเอียดที่แย่กว่า float16 ราว 8 เท่า · <strong>gradient จึงไม่ underflow ไม่ต้องทำ loss scaling</strong> เหตุผลที่งานเทรนสมัยใหม่เลือกตัวนี้"
  },
  "lossscaling": {
    term: "Loss scaling",
    en: "loss scaling",
    def: "คูณ loss ด้วยค่าคงที่ก่อน <code>backward()</code> แล้วหารกลับหลังจากนั้น · ใช้กับ <code>float16</code> เพราะ gradient ที่ต่ำกว่า <code>6.10e-05</code> จะถูกปัดเป็นศูนย์ และพารามิเตอร์ตัวนั้นจะหยุดเรียนถาวร"
  },
  "cancellation": {
    term: "Catastrophic cancellation",
    en: "catastrophic cancellation",
    def: "ลบสองค่าที่ใกล้กันมาก หลักที่ตรงกันหักล้างกันหมด เหลือแต่หลักท้ายที่เป็นความคลาดล้วน · วัดแล้ว <code>cos(1.0) - cos(1.0+1e-7)</code> ใน float32 ผิดไป <strong>29%</strong> ทั้งที่ตัวตั้งทั้งสองแม่น 7 หลัก"
  }
};
